...
const HandleEditMembers = ({
    ...
    const [userState, setUserState] = useState<Members>(user);
    const [initialUserState] = useState<Members>({ ...user });

    const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
        e.preventDefault();

        const changes: Partial<Members> = Object.keys(userState).reduce<Partial<Members>>(
            (acc, key) => {
                const typedKey = key as keyof Members;
                if (userState[typedKey] !== initialUserState[typedKey]) {
                    // FIXME : Fix the any type here
                    acc[typedKey] = userState[typedKey] as any;
                }
                return acc;
            },
            {} as Partial<Members>
                        ...
                        <ModalBody>
                            ...
                            <Input
                                ...
                                onChange={(e) =>
                                    setUserState({
                                        ...userState,
                                        numberAndStreet: e.target.value,
                                    })
                                }
                            />
                            <div className="flex gap-2">
                                <Input
                                    ...
                                    isRequired
                                    onChange={(e) =>
                                        setUserState({ ...userState, postalCode: e.target.value })
                                    }
                                />
                                <Input
                                    label="Ville"
